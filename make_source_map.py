...
if args.obs_ids is not None:
    ...
    obslist = ctx.obsdb.query(f"type=='obs' and subtype=='cal' and {source} and start_time > {start_time} and stop_time < {cfg['stop_time']}", tags=[f"{source}=1"])
...
res = cfg.get("res", (10./3600.)*np.pi/180.)
mask = cfg.get("mask", {'shape': 'circle', 'xyr': (0, 0, .5)})
...
for i, obs in enumerate(obslist):
    ...
    for ufm in ufms:
        ...
        for band in np.unique(bp):
            ...
            [[dec_min, ra_min ], [dec_max, ra_max]] = 3600*np.rad2deg(out["solved"].corners(corner=False))
            ...
            plt.xlim((ra_min - pixsize*cent[1]-extent, ra_min - pixsize*cent[1]+extent))
            plt.ylim((dec_min + pixsize*cent[0]-extent, dec_min + pixsize*cent[0]+extent))
            plt.savefig(os.path.join(obs_plot_dir, f"{obs['obs_id']}_{ufm}_{band_name}_map.png"))
            plt.xlim((ra_min - pixsize*cent[1]-extent/zoom, ra_min - pixsize*cent[1]+extent/zoom))
            plt.ylim((dec_min + pixsize*cent[0]-extent/zoom, dec_min + pixsize*cent[0]+extent/zoom))
            plt.savefig(os.path.join(obs_plot_dir, f"{obs['obs_id']}_{ufm}_{band_name}_map_zoom.png"))
            ...
            plt.title(f"{obs['obs_id']}_{ufm}_{band_name}")
            plt.xlim((0, extent))
            plt.savefig(os.path.join(obs_plot_dir, f"{obs['obs_id']}_{ufm}_{band_name}_prof.png"), bbox_inches='tight')
            plt.xlim((0, extent/zoom))
            plt.savefig(os.path.join(obs_plot_dir, f"{obs['obs_id']}_{ufm}_{band_name}_prof_zoom.png"), bbox_inches='tight')
            ...
            plt.xlim((ra_min - pixsize*cent[1]-extent/zoom, ra_min - pixsize*cent[1]+extent/zoom))
            plt.ylim((dec_min + pixsize*cent[0]-extent/zoom, dec_min + pixsize*cent[0]+extent/zoom))
            plt.savefig(os.path.join(obs_plot_dir, f"{obs['obs_id']}_{ufm}_{band_name}_map_log10_zoom.png"))
            ...
            plt.title(f"{obs['obs_id']}_{ufm}_{band_name} log")
            plt.xlim((0, extent))
            plt.savefig(os.path.join(obs_plot_dir, f"{obs['obs_id']}_{ufm}_{band_name}_prof_log10.png"), bbox_inches='tight')
            plt.xlim((0, extent/zoom))
            plt.savefig(os.path.join(obs_plot_dir, f"{obs['obs_id']}_{ufm}_{band_name}_prof_log10_zoom.png"), bbox_inches='tight')
            ...
            if plot_smooth:
                ...
                axd['A'].imshow(smoothed[cent[0]-extent_pix:cent[0]+extent_pix, cent[1]-extent_pix:cent[1]+extent_pix], origin="lower")
