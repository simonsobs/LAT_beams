...
def svd_source_flag(aman, cfg, logger, info):
    ...
    common_mode, source_flag = iter_svd(
        aman.signal, cfg.svd_modes, cfg.n_std, cfg.svd_iters, True
    )
    if cfg.iter_svd_sub:
...
def get_source_flag(aman, source_flag_exp, cfg, logger, info, buffer) -> RangesMatrix:
    flags = {}
    words = WORD_PATTERN.findall(source_flag_exp)
    if len(words) == 0:
        return RangesMatrix.ones(aman.signal.shape)
    for word in words:
        if word in aman:
    ...
    flags = {
        k: match_shape(v, aman.signal.shape).buffer(int(buffer // 2))
        for k, v in flags.items()
    }
    expression_for_eval = WORD_PATTERN.sub(
        lambda match: f"flags[{match.group()!r}]", source_flag_exp
    )
    ...
    with MPICommExecutor(local_comm, 0) as executor:
        if executor is not None:
            ...
            for i, j in enumerate(joblist):
                ...
                logger.debug("TOD is of %s", source)
                ...
                aman.wrap("sig_filt", sig_filt, [(0, "dets"), (1, "samps")])
                ...
                logger.debug("TOD downsampled to shape %s", aman.signal.shape)
                ...
                std_est = tod_ops.jumps.std_est(np.asarray(aman.sig_filt), ds=1)
                ...
                if cfg.source_flag_exp != "" and (
                    ...
                    (
                        ...
                        info["res"],
                        info["mask"],
                    ) = (source_name, nominal, ufm, cfg.res, cfg.pointing_mask)
                    source_flag = get_source_flag(
                        aman, cfg.source_flag_exp, cfg, logger, info, cfg.block_size
                    )
                    if cfg.filter_for_sources:
                        ...
                        for b in np.unique(np.asarray(aman.bp)):
                            bmsk = aman.bp == b
                            aman.signal[bmsk] = cp.filter_for_sources(
                                tod=aman.restrict("dets", bmsk, in_place=False),
                                signal=aman.signal[bmsk],
                                source_flags=source_flag[bmsk],
                ...
                for band in np.unique(np.asarray(aman_full.bp)):
                    ...
                    if aman.dets.count < cfg.min_dets:
                        ...
                        msg += _msg
                        continue
                    logger.log(25, "%s detectors after ptp cuts", aman.dets.count)
