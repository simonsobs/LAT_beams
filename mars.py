...
plt.plot(aman.signal[::3].T, alpha=.3)
...
if os.path.isfile("mars_fp.h5"):
    ...
    focal_plane = pointing_quickfit(aman, (4, None), fwhm=np.deg2rad(1.5/60.), source='mars', bin_priors=True)
    aman.wrap("focal_plane", focal_plane)
    med_xi = np.median(aman.focal_plane.xi)
    med_eta = np.median(aman.focal_plane.eta)
    msk = (np.sqrt((aman.focal_plane.xi - med_xi)**2 + np.abs(aman.focal_plane.eta - med_eta)**2) < np.deg2rad(.5)) * (aman.focal_plane.amp > 0)
