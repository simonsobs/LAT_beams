...
from lat_beams.plotting import auto_relplot, plot_map
...
for split in cfg.split_by:
    ...
    for spl_dir in sorted([f.path for f in os.scandir(data_dir_spl) if f.is_dir()]):
        ...
        for epoch in cfg.epochs:
            ...
            mod_path = os.path.join(
                ...
                split,
                spl_rel,
                f"model_profile_{spl_rel}_{epoch[0]}_{epoch[1]}.txt",
            )
            ...
            mod_path = os.path.join(
                ...
                split,
                spl_rel,
                f"model_window_{spl_rel}_{epoch[0]}_{epoch[1]}.txt",
            )
    ...
    plot = auto_relplot(
        ...
        kind="line",
        estimator=None,
        hue=("band" if "tube_slot" in split else None),
    )
    ...
    if split == "band":
        for b in ["f090", "f150", "f220"]:
            act_te = np.genfromtxt(
                f"/global/cfs/cdirs/cmb/data/act_dr6/dr6.02/beams/leakage_beams/nominal/pa{4 if b == 'f220' else 6}_{b}_gamma_t2e.txt"
            )
            act_main = np.genfromtxt(
                f"/global/cfs/cdirs/cmb/data/act_dr6/dr6.02/beams/main_beams/nominal/coadd_pa{4 if b == 'f220' else 6}_{b}_night_beam_tform_jitter_cmb.txt"
            )
            to_plot["ell"] += act_te[:, 0].tolist()
            to_plot["te_leakage"] += (
                act_te[:, 1] * act_main[:, 1] / act_main[0, 1]
            ).tolist()
