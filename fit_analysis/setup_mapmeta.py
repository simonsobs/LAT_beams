...
for file in files:
    ...
    obslist = context.obsdb.query(
        f"type=='obs' and subtype=='cal' and obs_id=='obs_{result['obs_id']}_{result['tube_slot']}_{result['wafers_present']}'"
    )

    if len(obslist) > 1:
        raise Exception(
            f"More than one observation matches id obs_{result["obs_id"]}_{result["tube_slot"]}_{result["wafers_present"]}"
        )
    ...
    if epoch == "N/A" and epochs:
        print(
            f"Warning! obs_{result["obs_id"]}_{result["tube_slot"]}_{result["wafers_present"]} is not within defined epochs! ({ctime})"
        )
