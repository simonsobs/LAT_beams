...
def plot_fit_result(T, fit, residual=None, abs_log = True, figsize=(3*3, 2*3), figsize2=(8,4),
                   circle_radius=None, circle_center=None, savename=None, title=None, zoom=None):
    if residual is None:
        residual = T - fit
    ...
    ax[1][0].imshow(T, vmin=vmin, vmax=vmax*.05, origin='lower', cmap='viridis')
    ax[1][0].set_title("Data (5% vmax)")
    ax[1][1].imshow(fit, vmin=vmin, vmax=vmax*.05, origin='lower', cmap='viridis')
    ax[1][1].set_title("Fit (5% vmax)")
    ax[1][2].imshow(residual, vmin=vmin, vmax=vmax*.05, origin='lower', cmap='viridis')
    ax[1][2].set_title("Residual (5% vmax)")
    ...
    if zoom is not None:
        ...
        ax[0][0].set_ylim(y0, yf)
        ax[0][1].set_xlim(x0, xf)
        ax[0][1].set_ylim(y0, yf)
        ax[0][2].set_xlim(x0, xf)
        ax[0][2].set_ylim(y0, yf)
        ax[1][0].set_xlim(x0, xf)
        ax[1][0].set_ylim(y0, yf)
        ax[1][1].set_xlim(x0, xf)
        ax[1][1].set_ylim(y0, yf)
        ax[1][2].set_xlim(x0, xf)
        ax[1][2].set_ylim(y0, yf)
