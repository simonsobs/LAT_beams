...
class BaseModel:
    ...
    def fit(
        self, mapdata, sigma=None, p0=None, bounds=None, mask=None, abs_sigma=False
    ):
...
class EllipticGaussian(BaseModel):
    ...
    def func(self, coords, amp, mux, muy, ellip, theta_fwhm, phi, offset):
        ...
        g = (
            amp * np.exp(-0.5 * ((xp / sigma_maj) ** 2 + (yp / sigma_min) ** 2))
            + offset
        )
        return g.ravel()
    ...
    def bounds(self, mapdata):
        ...
        bounds = (
            [
                ...
                -np.pi / 2,
                -np.inf,
            ],  # we try to avoid fitting rho too close to bounds
