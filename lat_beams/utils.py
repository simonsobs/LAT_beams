...
def recenter(imap, obs_id, stream_id, band, fit_file, norm=True, extent=None):
    ...
    cent = enmap.sky2pix(imap.shape, imap.wcs, np.array((aman.eta0.to(u.rad).value, aman.xi0.to(u.rad).value)))
    ...
    if extent is not None:
        imap = crop_maps([imap], enmap.sky2pix(imap.shape, imap.wcs, np.array((0, 0))), extent/np.abs(3600*imap.wcs.wcs.cdelt[1]))[0]
