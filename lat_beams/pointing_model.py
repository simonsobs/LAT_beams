...
def transform(x, az, el, roll, q_data):
    ...
    rhs = (
        q_lonlat * q_mir_center * q_el_roll * q_el_axis_center * q_cr_roll * q_cr_center
    )
...
def joint_transform(x, tmsks, pmsks, az, el, roll, q_data):
    ...
    for tmsk, pmsk in zip(tmsks, pmsks):
        if len(pmsk) == 0:
            continue
        xi0[tmsk], eta0[tmsk] = transform(
            x[pmsk],
            az[tmsk],
            el[tmsk],
            roll[tmsk],
            quat.G3VectorQuat(np.array(q_data)[tmsk]),
        )
    return xi0, eta0
...
def fit_func(x, tmsks, pmsks, az, el, roll, q_data):
    ...
    rms = 3600 * np.sqrt(
        np.nanmean(np.hstack([np.rad2deg(xi0), np.rad2deg(eta0)]) ** 2)
    )
    return rms
...
def fit(time_ranges, ctime, az, el, roll, q_data, d):
    ...
    for t0, t1, to_fit, indep_list in time_ranges:
        ...
        if not to_fit:
            tmsks += [tmsk]
            continue
        tmsk *= np.abs(d - np.median(d[tmsk])) < np.deg2rad(0.25)
        tmsks += [tmsk]
        if np.sum(np.isin(indep_list, par_list)) != len(indep_list):
            raise ValueError(
                f"Invalid independant parameters in time range starting with {t0}"
            )
    ...
    for t0, t1, to_fit, indep_list in time_ranges:
        ...
        if np.sum(pmsk) != len(default_params):
            raise ValueError(
                f"Time range starting with {t0} somehow has the wrong number of parameters!"
            )
    ...
    res = minimize(
        fit_func,
        x,
        args=(tmsks, pmsks, az, el, roll, q_data),
        bounds=bounds,
        method="Nelder-Mead",
        options={"adaptive": True},
    )  # jac="3-point", method="trust-constr") #, ) #, method="TNC", options={"maxfun":10000})
    print(res)
