...
def guess_tube(telescope, wafers):
    ...
    info = info.subset(keys=['wafer_slot'], rows=info['tube_slot'] == tube_slot).distinct()
...
if __name__ == '__main__':
    ...
    parser.add_argument('export_dirs', nargs='+',
            ...
            if args.dichroic_sub:
                ...
                for w in wafers_found:
                    ...
                    if detset not in detsets:
                        ...
                        detsets[detset] = [props.subset(rows=det_select), fp.subset(rows=det_select)]
                    ...
                    wafer_file = os.path.join(practical_path,
                                              os.path.split(from_here)[1])
                    obsfiledb.add_obsfile(wafer_file,
                                          obs_id, detset, 0, 1)
                ...
                obsfiledb.add_obsfile(os.path.join(practical_path,
    ...
    open(f'{args.context_dir}/context.yaml', 'w').write(yaml.dump(context, sort_keys=False))
