...
for obs_id in dat.keys():
    ...
    for ot in dat[obs_id].optics_tubes: 
        for fp in ot.focal_planes:
            n_det = np.sum(np.isfinite(fp.weights) * (fp.weights > .5))
            times += [time]
            ufms += [fp.stream_id]
            n_dets += [n_det]
...
plt.scatter(times, ufms, c=xi_scales, vmin=.9, vmax=1.1)
elines = np.unique(np.array(epochs))[:-1]
for el in elines:
    plt.axvline(el)
...
plt.scatter(times, ufms, c=eta_scales, vmin=.9, vmax=1.1)
elines = np.unique(np.array(epochs))[:-1]
for el in elines:
    plt.axvline(el)
plt.colorbar(label="scale")
...
plt.hist(xi_scales[times > epochs[-1][0]], bins='auto', alpha=.5, label="Xi")
plt.hist(eta_scales[times > epochs[-1][0]], bins='auto', alpha=.5, label="Eta")
plt.legend()
plt.xlim((1, None))
