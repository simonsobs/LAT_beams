...
msk *= (cross_sa < np.percentile(cross_sa[msk], 90)) * (
    cross_sa > np.percentile(cross_sa[msk], 1)
)
...
for split in cfg.split_by:
    ...
    plt.scatter(all_fits["time"], split_vec, c=cross_sa, alpha=0.4)
    ...
    for spl in spls:
        smsk = split_vec == spl
        plt.scatter(all_fits["time"][smsk], cross_sa[smsk], alpha=0.4)
        for e in epoch_lines:
            plt.axvline(e)
    ...
    for start, end in cfg.epochs:
        ...
        for spl in np.unique(tvec):
            ...
            for name, xax, dat in to_scatter:
                ...
                plt.title(
                    f"{spl} Fractional Octopole Solid Angle by {name.title()} ({start}, {end})"
                )
                plt.xlabel(xax)
                plt.ylabel("Fractional Octopole Solid Angle")
                plt.savefig(
                    os.path.join(
                        plot_dir_spl, f"{name.lower()}_scatter_{spl}_{start}_{end}.png"
                    ),
                    bbox_inches="tight",
                )
                plt.close()
